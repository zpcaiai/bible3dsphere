...
export function BibleMapClient() {
  ...
  useEffect(() => {
    const effective = layer === 'tribes' || layer === 'empires' ? layer : 'all'
    void getJson<BibleTerritoryDTO>(`/api/bible-map/territories?year=${year}&layer=${effective}`).then(setTerritories)
  }, [year, layer])

  useEffect(() => {
    setEventsLoading(true)
    void getJson<BibleMapEventDTO>(`/api/bible-map/events?year=${year}`).then((d) => {
      setEvents(d)
  ...
  useEffect(() => {
    void getJson<BibleProphecyDTO>('/api/bible-map/prophecies').then(setProphecies)
    void getJson<BibleCampaignDTO>('/api/bible-map/campaigns').then(setCampaigns)
  }, [])
