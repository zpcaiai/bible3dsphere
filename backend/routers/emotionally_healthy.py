"""EH router — 情商与灵命 (/api/eh). Scazzero《Emotionally Healthy Spirituality》六维自评。"""
...
try:
    from backend import emotionally_healthy_engine as engine
except Exception:  # pragma: no cover
    import emotionally_healthy_engine as engine  # type: ignore
try:
...
router = APIRouter(prefix="/api/eh", tags=["emotionally_healthy"])
_state: Dict[str, Any] = {}


def init_emotionally_healthy_router(*, get_db, release_db, get_session_user, to_shanghai_iso) -> None:
...
def assess(request: Request, body: AssessBody) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO eh_entries (id, email, text, crisis, analysis_json) "
                "VALUES (%s,%s,%s,%s,%s)",
...
def history(request: Request, limit: int = Query(default=20, ge=1, le=100)) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, crisis, created_at FROM eh_entries "
...
def latest(request: Request) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT analysis_json, created_at FROM eh_entries WHERE email=%s "
