"""Affections router — 宗教情感真伪辨 (/api/affections). Edwards《Religious Affections》。"""
...
try:
    from backend import affections_engine as engine
except Exception:  # pragma: no cover
    import affections_engine as engine  # type: ignore
try:
...
router = APIRouter(prefix="/api/affections", tags=["affections"])
_state: Dict[str, Any] = {}


def init_affections_router(*, get_db, release_db, get_session_user, to_shanghai_iso) -> None:
...
def assess(request: Request, body: AssessBody) -> dict:
    ...
    result = engine.assess(body.ratings, body.text, settings=_settings, use_ai=body.use_ai)
    conn = _state["get_db"]()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO affections_entries (id, email, input_text, crisis, analysis_json) "
                "VALUES (%s,%s,%s,%s,%s)",
...
def history(request: Request, limit: int = Query(default=20, ge=1, le=100)) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT id, crisis, created_at FROM affections_entries "
...
def latest(request: Request) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT analysis_json, created_at FROM affections_entries WHERE email=%s "
