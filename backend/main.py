...
async def _initialize_runtime(app: FastAPI) -> None:
    ...
    try:
        init_spiritual_planet_discernment_extended_router(
            get_db=_get_db,
            release_db=_release_db,
            get_session_user=_get_session_user,
            is_admin=_is_admin,
        )
        print('[routers] spiritual_planet_discernment_extended router initialized', flush=True)
    except Exception as exc:
        print(f'[routers] WARNING: spiritual_planet_discernment_extended router init failed: {exc}', flush=True)
