...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_competency_definitions','mission_competency_evidence','mission_observation_rubrics','mission_stage_certifications','mission_safeguarding_training_records'] LOOP EXECUTE format('DROP POLICY IF EXISTS mission_tenant_isolation ON %I',t);EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true) OR tenant_id IS NULL) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true) OR tenant_id IS NULL)',t);END LOOP;END$$;
