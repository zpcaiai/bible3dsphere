...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_digital_security_profiles','mission_device_inventory','mission_security_exceptions','mission_emergency_response_plans','mission_evacuation_plans','mission_emergency_drills','mission_deployment_readiness_gates'] LOOP EXECUTE format('DROP POLICY IF EXISTS mission_tenant_isolation ON %I',t);EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true)) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true))',t);END LOOP;END$$;
