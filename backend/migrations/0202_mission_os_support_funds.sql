...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_support_campaigns','mission_support_pledges','mission_support_receipts','mission_funds','mission_expense_requests','mission_expense_approvals','mission_financial_findings','mission_financial_disclosures'] LOOP EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true)) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true))',t);END LOOP;END$$;
