...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_medical_readiness_profiles','mission_medication_continuity_plans','mission_insurance_policies','mission_insurance_gap_assessments','mission_households','mission_family_readiness_plans','mission_spouse_readiness_reviews','mission_child_education_plans'] LOOP EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true)) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true))',t);END LOOP;END$$;
