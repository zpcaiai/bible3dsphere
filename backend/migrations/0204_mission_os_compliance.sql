...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_compliance_cases','mission_compliance_domains','mission_compliance_requirements','mission_professional_opinions','mission_tax_profiles','mission_data_transfer_assessments'] LOOP EXECUTE format('DROP POLICY IF EXISTS mission_tenant_isolation ON %I',t);EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true)) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true))',t);END LOOP;END$$;
