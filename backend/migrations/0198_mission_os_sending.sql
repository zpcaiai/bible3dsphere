...
DO $$DECLARE t TEXT;BEGIN FOREACH t IN ARRAY ARRAY['mission_church_confirmations','mission_church_confirmation_reviews','mission_candidate_applications','mission_application_sections','mission_sending_committees','mission_sending_committee_members','mission_sending_committee_meetings','mission_sending_committee_votes','mission_sending_decisions'] LOOP EXECUTE format('CREATE POLICY mission_tenant_isolation ON %I USING(tenant_id=current_setting(''app.tenant_id'',true)) WITH CHECK(tenant_id=current_setting(''app.tenant_id'',true))',t);END LOOP;END$$;
