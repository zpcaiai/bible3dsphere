...
def _patch_llm(monkeypatch, name, value):
    monkeypatch.setattr(llm, name, value)
    for module in (tm, diag, narr):
        bound = getattr(module, "_llm", None)
        if bound is not None:
            monkeypatch.setattr(bound, name, value, raising=False)


def test_degrades_when_enhance_returns_none(monkeypatch):
    _patch_llm(monkeypatch, "enhance", lambda *a, **k: None)
...
def test_truth_map_merges_prose_only_and_protects_scripture(monkeypatch):
    ...
    _patch_llm(monkeypatch, "generate_structured", lambda *a, **k: None)
    _patch_llm(monkeypatch, "enhance", lambda *a, **k: fake)
...
def test_suffering_engine_bypasses_worldview_llm(monkeypatch):
    ...
    calls = []
    _patch_llm(monkeypatch, "enhance", lambda *a, **k: (calls.append(1), {"x": 1})[1])
...
def test_diagnoser_structured_maps_into_contract(monkeypatch):
    _patch_llm(monkeypatch, "generate_structured", lambda system, payload, name, **k: {
        "summary": "结构化AI总结",
