    ...
    <div className="app-shell">
      <aside className="left-panel glass">
        <div className="brand-block">
          ...
          <h1>情绪云团球体</h1>
          <p>远看是情绪簇，近看是具体 emotion feature。点击点位可联动经文结果。</p>
        </div>

        <div className="install-card glass-subtle">
          <div className="section-title">Install App</div>
          <div className="muted">将当前 React 页面以 PWA 形式安装到手机主屏幕，获得更接近原生 App 的预览体验。</div>
          {canInstall ? (
        ...
        <div className="cluster-list glass-subtle">
          <div className="section-title">情绪簇概览</div>
      ...
      <aside className="right-panel glass">
        {guidance && (
          <section className="detail-section guidance-section">
              ...
              verseGroups.map((group) => (
                <div key={group.language} className="verse-group">
                  ...
                  {group.items.map((item) => (
                    <div key={item.pk_id} className="verse-card-ui">
        ...
        <section className="detail-section">
          ...
          <div className="history-list">
            {historyItems.slice(0, 8).map((item, idx) => (
              <button key={`${item.query_text}-${idx}`} className="history-item" onClick={() => setQuery(item.query_text)}>
