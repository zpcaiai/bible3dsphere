...
function CameraLODWatcher() {
  ...
  useFrame(() => {
    ...
    const lod = d > 5.5 ? 'far' : d > 3.5 ? 'mid' : 'near'
...
export function EmotionSphereScene({ onVerseTrigger }) {
  return (
    <SceneErrorBoundary>
      <Canvas style={{ width: '100%', height: '100%', display: 'block' }} camera={{ position: [0, 0, 6.5], fov: 55 }} dpr={[1, 2]}>
        <color attach="background" args={['#060b18']} />
        <fog attach="fog" args={['#060b18', 6, 14]} />
        <ambientLight intensity={0.8} />
        <directionalLight position={[5, 7, 4]} intensity={1.3} />
        <pointLight position={[-6, -5, -3]} intensity={1.1} color="#5577ff" />
        <Stars radius={28} depth={22} count={2000} factor={2.8} saturation={0} fade speed={0.3} />
